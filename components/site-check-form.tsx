  ...
  if (state?.success) {
    return (
      <div className="rounded-xl border border-trust/30 bg-trust/5 p-6 text-center">
        <p className="text-lg font-bold text-trust">✓ Request Submitted</p>
  ...
  return (
    <form action={formAction} className="space-y-4">
      ...
      <div>
        ...
        <input type="text" id="name" name="name" required minLength={2} className="w-full rounded-md border border-border bg-navy-900 px-4 py-3 text-white placeholder-slate-600 focus:border-accent focus:outline-none" placeholder="Your name" />
      ...
      <div>
        ...
        <input type="tel" id="phone" name="phone" required className="w-full rounded-md border border-border bg-navy-900 px-4 py-3 text-white placeholder-slate-600 focus:border-accent focus:outline-none" placeholder="04xx xxx xxx" />
      ...
      <div>
        ...
        <input type="text" id="suburb" name="suburb" required minLength={2} className="w-full rounded-md border border-border bg-navy-900 px-4 py-3 text-white placeholder-slate-600 focus:border-accent focus:outline-none" placeholder="e.g. Joondalup" />
        {state?.errors?.suburb && <p className="mt-1 text-sm text-red-400">{state.errors.suburb}</p>}
      </div>

      <button type="submit" disabled={isPending} className="w-full rounded-md bg-cta py-4 text-sm font-bold uppercase tracking-wide text-navy-950 transition-colors hover:bg-amber-400 disabled:opacity-50">
        {isPending ? "Submitting..." : "Book Your Free Site Check →"}
      </button>
    </form>
  );
}
